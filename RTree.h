...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback(DATATYPE a_data, void* a_context), void* a_context);
...
public:
  ...
  struct Node
  {
	...
	//Addtional Index
	 S_Rectangle rect;
	 Node* parent;
     SortedInvertedList<int,int,float> sil_invertedlists;
	 Node(){
		parent=NULL;
	 }
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback(DATATYPE a_data, void* a_context), void* a_context);
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback(DATATYPE a_data, void* a_context), void* a_context)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount,t_resultCallback a_resultCallback(DATATYPE a_data, void* a_context), void* a_context)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
